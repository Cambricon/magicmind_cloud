...
bool CheckModel(magicmind::IModel *model) {
  if (model->GetInputNum() != 1) {
    std::cout << "Input number is [" << model->GetInputNum() << "]."
              << std::endl;
    return false;
  }
  if (model->GetOutputNum() != 1) {
    std::cout << "Output number is [" << model->GetOutputNum() << "]."
              << std::endl;
    return false;
  }
  if (model->GetInputDimension(0)[3] != 3) {
    std::cout << "Input channels [" << model->GetInputDimension(0)[3] << "]."
              << std::endl;
    return false;
  }
  if (model->GetInputDataType(0) != magicmind::DataType::UINT8) {
    std::cout << "Input data type is [" << model->GetInputDataType(0) << "]."
              << std::endl;
    return false;
  }
  if (model->GetOutputDataType(0) != magicmind::DataType::FLOAT32) {
    std::cout << "Output data type is [" << model->GetOutputDataType(0) << "]."
              << std::endl;
    return false;
  }
  return true;
