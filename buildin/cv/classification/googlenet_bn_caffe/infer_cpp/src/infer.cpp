...
int main(int argc, char **argv) {
  ...
  TimeCollapse time_googlenet_caffe("infer googlenet_caffe");

  // create an instance of ModelRunner
  auto googlenet_runner = new ModelRunner(FLAGS_device_id, FLAGS_magicmind_model);
  if (!googlenet_runner->Init(FLAGS_batch_size)) {
    SLOG(ERROR) << "Init googlenet runnner failed.";
    return false;
  }
  // load images
  ...
  auto batch_data = googlenet_runner->GetHostInputData();
  // one batch input data addr offset
  int batch_image_offset = googlenet_runner->GetInputSizes()[0] / FLAGS_batch_size;
  ...
  for (int i = 0; i < image_num; i++) {
    ...
    if (batch_counter % real_batch_size == 0) {
      // copy in
      googlenet_runner->H2D();
      // compute
      googlenet_runner->Run(FLAGS_batch_size);
      // copy out
      googlenet_runner->D2H();
      // get model's output addr in host
      auto host_output_ptr = googlenet_runner->GetHostOutputData();
