 ...
 */
result *LoadImages(const std::string image_dir, int batch_size, int image_num,
  ...
  if (pad_num != batch_size) {
    std::cout << "There are " << test->image_paths.size()
              << " images in total, add " << pad_num
...
cv::Mat Preprocess(cv::Mat img, const magicmind::Dims &input_dim) {
  ...
  */
  ...
  if (h > FLAGS_new_size || w > FLAGS_new_size) {
    std::cout << "new_size[" << FLAGS_new_size << "] less than input_dim["
              << input_dim << "]." << std::endl;
  }
  // resize
  ...
  cv::resize(
      img, resized,
      cv::Size(std::round(scale * img.cols), std::round(scale * img.rows)));
  // center crop
  auto roi = resized(cv::Rect((resized.cols - FLAGS_new_size) / 2,
                              (resized.rows - FLAGS_new_size) / 2, w, h));
