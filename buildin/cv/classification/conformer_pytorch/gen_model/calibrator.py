...
class CalibData(mm.CalibDataInterface):
    ...
    def preprocess_images(self, data_begin: int, data_end: int) -> np.ndarray:
        ...
        for i in range(data_begin, data_end):
            ...
            img = cv2.resize(img, (round(scale * origin_w), round(scale * origin_h)), interpolation=cv2.INTER_CUBIC)
            # center crop
            crop_size = 224
            roi_y1 = round((img.shape[0] - crop_size) / 2)
            roi_y2 = roi_y1 + dst_h
            roi_x1 = round((img.shape[1] - crop_size)  / 2)
            roi_x2 = roi_x1 + dst_w
