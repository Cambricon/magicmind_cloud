...
DEFINE_int32(input_size, 224, "The resized image size.");
DEFINE_bool(input_rgb, true, "Input convert to rgb or not.");

void UnpackTensorList(std::vector<magicmind::IRTTensor *> *tensors) {
  if (!tensors) {
    return;
  }
  for (int i = 0; i < tensors->size();) {
    if ((*tensors)[i]->GetDataType() == magicmind::DataType::TENSORLIST) {
      std::vector<magicmind::IRTTensor *> unpack_list;
      auto iter = tensors->begin() + i;
      MM_CHECK(GetIRTTensorList(*iter, &unpack_list));
      (*iter)->Destroy();
      tensors->erase(iter);
      tensors->insert(tensors->begin() + i, unpack_list.begin(), unpack_list.end());
    } else {
      ++i;
    }
  }
...
int main(int argc, char **argv) {
    ...
    float *output_data_ptr_1 = new float[FLAGS_batch * classes];
    float *output_data_ptr_2 = new float[FLAGS_batch * classes];
    ...
    for (int i = 0; i <= image_num; i++) {
	    ...
	    UnpackTensorList(&output_tensors);
	    CNRT_CHECK(cnrtQueueSync(queue));

            // 9. copy out
            CNRT_CHECK(cnrtMemcpy(
                output_data_ptr_1, output_tensors[0]->GetMutableData(),
                output_tensors[0]->GetSize(), CNRT_MEM_TRANS_DIR_DEV2HOST));
            CNRT_CHECK(cnrtMemcpy(
                output_data_ptr_2, output_tensors[1]->GetMutableData(),
                output_tensors[1]->GetSize(), CNRT_MEM_TRANS_DIR_DEV2HOST));
            for (int count = 0; count < output_tensors[1]->GetSize()/sizeof(float); count++) {
                output_data_ptr[count] = output_data_ptr_1[count] + output_data_ptr_2[count];
            }
            // post-process
            for (int _batch = 0; _batch < FLAGS_batch; _batch++) {
                ...
                // get top5
                std::vector<int> top5 = ArgTopK(output_data_ptr + _batch * classes, classes, 5);
                ...
                for (int j = 0; j < 5; j++) {
                    if (!FLAGS_result_top5_file.empty()) {
                        result_top5_file.write("[" + std::to_string(i - FLAGS_batch + _batch) +
                ...
                if (i == image_num && _batch == (FLAGS_image_num % FLAGS_batch - 1)) break;
            }
        }
        ...
        if (img.data != NULL) {
            ...
            cv::Mat dst_img = Preprocess(img, input_dim, FLAGS_input_size, FLAGS_input_rgb);
