...
cv::Mat Preprocess(cv::Mat img, const magicmind::Dims &input_dim, int input_size, bool input_rgb) {
    ...
    */
    ...
    int w = input_dim[2];
    if (input_size > 32) {
        float scale = 233.0 / std::min(img.cols, img.rows);
        // resize
        cv::Mat resized;
        cv::resize(img, resized, cv::Size(std::round(scale * img.cols), std::round(scale * img.rows)), cv::INTER_CUBIC);
        // center crop
        img = resized(cv::Rect(std::round((resized.cols - input_size) / 2.0), std::round((resized.rows - input_size) / 2.0), input_size, input_size));
    }
