...
def pytorch_parser(args):
    ...
    if args.precision.find('qint') != -1:
        all_node = network.get_all_nodes_in_network()
        # 需要提升精度的算子列表
        op_list = ['conv', 'matmul']
        #'Div_', 'Erf_'
        for node in all_node:
            name = node.get_node_name()
            if is_contain(name, op_list):
                for i in range(node.get_input_count()):
                    if node.get_input(i).get_data_type() in [mm.DataType.FLOAT32]:
                        assert node.set_precision(i, mm.DataType.FLOAT16).ok()
                for i in range(node.get_output_count()):
                    if node.get_output(i).get_data_type() in [mm.DataType.FLOAT32]:
                        assert node.set_output_type(i, mm.DataType.FLOAT16).ok()
    # 设置网络输入数据形状
...
def main():
    ...
    builder = mm.Builder()
    assert builder != None
    model = builder.build_model("volo_d2_model", network, config)
    assert model != None
    # 将模型序列化为离线文件
