...
MM_RUN(){
    ...
    ${MM_RUN_PATH}/mm_run --magicmind_model $PROJ_ROOT_PATH/data/models/squeezenet_v1_0_caffe_model_${QUANT_MODE}_${SHAPE_MUTABLE}_${BATCH_SIZE} \
                          --iterations 1000 \
                          --batch ${BATCH} \
                          --threads ${THREADS} \
    ...
    do
      ...
      do 
        ...
        do
          ...
          python $MAGICMIND_CLOUD/test/compare_perf.py --output_file $PROJ_ROOT_PATH/data/output/${quant_mode}_true_${batch}_log_perf --output_ok_file $PROJ_ROOT_PATH/data/output_ok/${quant_mode}_true_${batch}_log_perf --model squeezenet_v1_0_caffe
        done
      done
    done
fi
