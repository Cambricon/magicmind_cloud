...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < image_num; i++) {
    ...
    CNRT_CHECK(cnrtMemcpy(input_tensors[0]->GetMutableData(), dst_img.data,
    ...
    CNRT_CHECK(cnrtMemcpy((void *)classification_output,
                          output_tensors[0]->GetMutableData(),
                          elem_data_count * 4, CNRT_MEM_TRANS_DIR_DEV2HOST));

    // get FLAGS_result_label_file
    if (!FLAGS_result_label_file.empty()) {
      result_label.write(
          "[" + std::to_string(i) + "]: " + std::to_string(test->labels[i]),
          false);
    }
    // get top1 and top5
    ...
    if (!FLAGS_result_top1_file.empty()) {
      result_top1_file.write(
          "[" + std::to_string(i) + "]: " + std::to_string(top5[0]), false);
    }
    ...
    for (int j = 0; j < 5; j++) {
      if (!FLAGS_result_top5_file.empty()) {
        result_top5_file.write(
            "[" + std::to_string(i) + "]: " + std::to_string(top5[j]), false);
      }
      if (!FLAGS_result_file.empty()) {
        result_file.write(std::to_string(j) + " [" + name_map[top5[j]] + "]",
                          false);
      }
    }
  }
