...
int main(int argc, char **argv) {
  ...
  TimeCollapse time_mobilenetv2("infer mobilenetv2");

  // create an instance of ModelRunner
  auto mbnetv2_runner = new ModelRunner(FLAGS_device_id, FLAGS_magicmind_model);
  if (!mbnetv2_runner->Init(FLAGS_batch_size)) {
    SLOG(ERROR) << "Init mbnetv2 runnner failed.";
    return false;
  }
  ...
  auto batch_data = mbnetv2_runner->GetHostInputData();
  // one batch input data addr offset
  int batch_image_offset = mbnetv2_runner->GetInputSizes()[0] / FLAGS_batch_size;
  ...
  for (int i = 0; i < image_num; i++) {
    ...
    if (batch_counter % real_batch_size == 0) {
      ...
      mbnetv2_runner->H2D();
      // compute
      mbnetv2_runner->Run(FLAGS_batch_size);
      // copy out
      mbnetv2_runner->D2H();
      // get model's output addr in host
      auto host_output_ptr = mbnetv2_runner->GetHostOutputData();
