...
int main(int argc, char **argv) {
  ...
  int img_w = 224, img_h = 224, img_c = 3, batch_size = FLAGS_batch_size,
      n_classes = 1000;
  // Set Input data ptr
    ...
    if (i != 0 && i % batch_size == 0) {
      ...
      for (int _batch = 0; _batch < batch_size; _batch++) {
        ...
        if (!FLAGS_result_label_file.empty()) {
          result_label.write("[" + std::to_string(i - batch_size + _batch) +
                                 "]: " + std::to_string(image_label[_batch]),
                             false);
        }
        std::vector<int> top5 =
            ArgTopK(output_data_ptr + _batch * n_classes, n_classes, 5);
        if (!FLAGS_result_top1_file.empty()) {
          result_top1_file.write("[" + std::to_string(i - batch_size + _batch) +
        ...
        for (int j = 0; j < 5; j++) {
          if (!FLAGS_result_top5_file.empty()) {
            result_top5_file.write("[" +
                                       std::to_string(i - batch_size + _batch) +
