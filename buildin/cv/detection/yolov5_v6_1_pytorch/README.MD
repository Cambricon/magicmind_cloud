...
bash run.sh force_float32 true 1 0.001 0.65 300
```

结果：

```bash
Generate model done, model save to /mm_ws/proj/modelzoo/magicmind_cloud/buildin/cv/detection/yolov5_v6_1_pytorch/data/models/yolov5_pytorch_model_force_float32_true
```
...
python $UTILS_PATH/compute_coco_mAP.py --file_list $UTILS_PATH/coco_file_list_5000.txt \
                                       ...
                                       --data_type val2017 \
                                       --json_name $PROJ_ROOT_PATH/data/output/yolov5_force_float32_true_1.json \
                                       --language infer_cpp \
                                       --image_num 1000
...
一键运行 benchmark 里的脚本跑出 yolov5m 在 COCO2017 数据集 中 MAP 如下：

| Model  | Precision           | Batch_Size | mAP (0.5:0.95) | mAP (0.5) |
| ------ | ------------------- | ---------- | -------------- | --------- |
| YoloV5 | force_float32       | 1          | 0.451          | 0.638     |
| YoloV5 | force_float16       | 1          | 0.446          | 0.637     |
| YoloV5 | qint8_mixed_float16 | 1          | 0.421          | 0.620     |
