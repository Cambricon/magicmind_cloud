...
int main(int argc, char **argv) {
  ...
  TimeCollapse time_yolov5("infer yolov5");

  // create an instance of ModelRunner
  auto yolov5_runner = new ModelRunner(FLAGS_device_id, FLAGS_magicmind_model);
  if (!yolov5_runner->Init(FLAGS_batch_size)) {
  ...
  auto batch_data = yolov5_runner->GetHostInputData();

  // one batch input data addr offset
  int batch_image_offset = yolov5_runner->GetInputSizes()[0] / FLAGS_batch_size;

  auto input_dim = yolov5_runner->GetInputDims()[0];
  int h = input_dim[1];
  int w = input_dim[2];
  ...
  for (int i = 0; i < image_num; i++) {
    ...
    if (batch_counter % real_batch_size == 0) {
      ...
      yolov5_runner->H2D();
      // compute
      yolov5_runner->Run(FLAGS_batch_size);
      // copy out
      yolov5_runner->D2H();
      // get model's output addr in host
      auto host_output_ptr = yolov5_runner->GetHostOutputData();
      // yolov5 model has two outputs
      auto detection_box = (float *)host_output_ptr[0];
      auto detection_num = (int *)host_output_ptr[1];

      magicmind::DataType box_dtype = yolov5_runner->GetOutputDataTypes()[0];
      // one batch detection box data addr offset
      int batch_box_offset =
          yolov5_runner->GetOutputSizes()[0] / FLAGS_batch_size / sizeof(box_dtype);
