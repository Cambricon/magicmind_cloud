...
bool post_process(cv::Mat &img, std::vector<std::vector<float>> results,
                  std::map<int, std::string> name_map, const std::string name,
                  const std::string output_dir, bool save_img) {
  ...
  for (int i = 0; i < detect_num; ++i) {
    ...
    file_map << name_map[detect_class] << "," << score << "," << xmin << ","
             << ymin << "," << xmax << "," << ymax << "\n";
    if (save_img) {
      ...
      cv::putText(img, text, cv::Point(xmin, ymin + text_size.height), fontface,
