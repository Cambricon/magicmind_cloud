...
cd $DATASETS_PATH
if [ ! -f "${DATASETS_PATH}/$FILE2" ];then
    wget -c https://www.crcv.ucf.edu/wp-content/uploads/2019/03/UCF101TrainTestSplits-RecognitionTask.zip --no-check-certificate -O $DATASETS_PATH/UCF101TrainTestSplits-RecognitionTask.zip
else
echo "$FILE2 Exist!"
fi

if [ ! -d "${DATASETS_PATH}/ucfTrainTestlist" ];then
    apt-get update
