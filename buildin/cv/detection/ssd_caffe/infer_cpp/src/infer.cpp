...
static std::vector<std::string> glabels = {"__background__",
                                           "aeroplane",
                                           "bicycle",
                                           "bird",
                                           "boat",
...
int main(int argc, char **argv) {
  ...
  //  CNRT_CHECK(cnrtMalloc(&output_mlu_addr_ptr, max_box * 7 *
  ...
  std::vector<std::string> image_paths =
      LoadImages(FLAGS_image_dir, FLAGS_batch);
  ...
  for (int i = 0; i < image_num; i++) {
    ...
    for (int t = 0; t < glabels.size(); t++) {
      std::string voc_preds_file = FLAGS_output_dir + "/" +
                                   "voc_preds/comp3_det_val_" + glabels[t] +
                                   ".txt";
      voc_preds_files.push_back(voc_preds_file);
    }
    post_process(img, results, glabels, voc_preds_files, image_name,
                 FLAGS_output_dir, FLAGS_save_img);
  }
