...
std::map<int, std::string> load_name(std::string name_map_file) {
  ...
  if (!in.is_open()) {
    std::cout << "Open label file failed. path : " << name_map_file
              << std::endl;
  }
  if (!in) {
 ...
 **/
template <typename T>
static std::vector<int64_t> ArgTopK(
    const T *data, int64_t idx_begin, int64_t idx_end, int64_t k,
    const std::function<bool(const T &, const T &)> &comp = std::greater<T>()) {
  if (idx_end - idx_begin < k) {
    std::cout << "ArgTopK: K is too large." << std::endl;
  }
  if (idx_end <= idx_begin) {
    std::cout << "ArgTopK: idx_end <= idx_begin." << std::endl;
  }
 ...
 **/
template <typename T>
static std::vector<T> TopK(
    const std::vector<T> &data, int64_t k,
    const std::function<bool(const T &, const T &)> &comp = std::greater<T>()) {
  if (static_cast<int64_t>(data.size()) < k) {
    std::cout << "TopK: K is too large." << std::endl;
  }
  std::vector<T> result;
 ...
 **/
static std::vector<int64_t> HeatMapArgTopK(const float *heatmap,
                                           const magicmind::Dims &dims,
                                           int64_t k) {
  ...
  for (int ci = 0; ci < num_cls; ++ci) {
    auto cls_topk_idxes =
        ArgTopK(heatmap, ci * cls_offset, (ci + 1) * cls_offset, clsk);
    memcpy(topk_idxes.data() + ci * clsk, cls_topk_idxes.data(),
           cls_topk_idxes.size() * sizeof(int64_t));
  }
...
std::vector<BBox> Postprocess(const std::vector<float *> &outputs,
                              const std::vector<magicmind::Dims> &output_dims,
                              int max_num, float thresholds) {
  if (output_dims.size() != 4) {
    std::cout << "There must be four output dimentations." << std::endl;
  }
  if (outputs.size() != 4) {
    std::cout << "There must be four outputs (heatmap_max, heatmap, wh, reg)."
              << std::endl;
  }
  ...
  cv::Mat hm_max_mat(heatmap_dims.GetElementCount() / heatmap_dims[0], 1,
                     CV_32FC1, heatmap_max);
  cv::Mat hm_mat(heatmap_dims.GetElementCount() / heatmap_dims[0], 1, CV_32FC1,
                 heatmap);
  cv::Mat keep;
  cv::compare(hm_max_mat, hm_mat, keep,
              cv::CMP_EQ);  // hm = (hm_max == hm) ? 255 : 0
  keep.convertTo(keep, CV_32F, 1.0 / 255);
  hm_mat = hm_mat.mul(keep);

  std::vector<int64_t> topk_idxes =
      HeatMapArgTopK(heatmap, heatmap_dims, max_num);
 ...
 */
void RescaleBBox(cv::Mat img, const magicmind::Dims &heatmap_dims,
                 std::vector<BBox> &bboxes,
                 std::map<int, std::string> imagenet_name_map,
  ...
  for (auto iter = bboxes.begin(); iter != bboxes.end();) {
    ...
    if (bbox.right <= bbox.left || bbox.bottom <= bbox.top) {
      ...
      iter++;
      file_map << imagenet_name_map[bbox.label] << "," << bbox.score << ","
...
void Draw(cv::Mat img, const std::vector<BBox> &bboxes,
  ...
  for (const auto &bbox : bboxes) {
    ...
    std::string text =
        imagenet_name_map[bbox.label] + ": " + std::to_string(bbox.score);
