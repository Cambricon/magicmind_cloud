...
static std::vector<RawBBox> PriorBox(int height, int width) {
  ...
  for (auto step : gkSteps) {
    feature_maps.emplace_back(std::make_pair(std::ceil(1.0 * height / step),
                                             std::ceil(1.0 * width / step)));
  }
  std::vector<RawBBox> anchors;
  for (size_t fm_id = 0; fm_id < feature_maps.size(); ++fm_id) {
    std::vector<int> min_size = {gkMinSizes[fm_id].first,
                                 gkMinSizes[fm_id].second};
...
static std::vector<RawBBox> Decode(const std::vector<const float *> &preds,
  ...
  CHECK_EQ(loc_dim[1], gkAnchors.size());  // shape immutable case, number of
                                           // anchors never changed.
  ...
  for (size_t i = 0; i < gkAnchors.size(); ++i) {
    ...
    for (int landm_id = 0; landm_id < num_landms; ++landm_id) {
      bbox.landms[landm_id].x =
          anchor.cx + landmst[landm_id * 2] * gkVariances[0] * anchor.w;
      bbox.landms[landm_id].y =
          anchor.cy + landmst[landm_id * 2 + 1] * gkVariances[0] * anchor.h;
    }
    bbox.score = conft[1];
...
static float IOU(const BBox &a, const BBox &b) {
  ...
  float overlap_area =
      std::max(0.f, right - left + 1) * std::max(0.f, bottom - top + 1);
...
void InitAnchors(int height, int width) { gkAnchors = PriorBox(height, width); }
...
std::vector<BBox> Postprocess(cv::Mat img, const magicmind::Dims &input_dim,
  ...
  float scaling_factors =
      std::min(1.0f * input_dim[1] / img.rows, 1.0f * input_dim[2] / img.cols);
