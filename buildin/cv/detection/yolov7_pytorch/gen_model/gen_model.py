...
def pytorch_parser(args):
    ...
    # 创建一个空的网络实例
    network = mm.Network()
    # 使用parser将Pytorch pt模型文件转换为MagicMind Network实例。
...
def generate_model_config(args):
    ...
    # 本例中使用的yolov7模型mean为0，std为255（即var=65025)。
...
def main():
    ...
    parser.add_argument("--pt_model", "--pt_model", type=str, default="../data/models/yolov7_traced_model.pt", help="modified yolov7 pt")
    parser.add_argument("--output_model", "--output_model", type=str, default="../data/models/yolov7_pytorch_model", help="save mm model to this path")
    ...
    # 生成模型
    builder = mm.Builder()
    model = builder.build_model("yolov7_pytorch_model", network, config)
    assert model != None
    # 将模型序列化为离线文件
