...
parser.add_argument(
    ...
    type=str,
    default="../data/models/yolov7_pytorch_model_qint8_mixed_float16_true_1",
)
parser.add_argument(
    "--image_dir",
    dest="image_dir",
...
parser.add_argument("--label_path", dest="label_path", type=str, default="coco.names")
parser.add_argument(
    "--output_dir", dest="output_dir", type=str, default="../data/images/output"
)
parser.add_argument(
    "--imgsz", dest="imgsz", default=640, type=int, help="inference size (pixels)"
)
parser.add_argument(
    "--batch_size", dest="batch_size", default=8, type=int, help="inference size (pixels)"
)
...
if __name__ == "__main__":
    ...
    if isinstance(args.imgsz, int):
        ...
        if(args.image_num == 0):
            log.error("args.image_num is 0, no need to infer.")
            exit()
        dataset = coco_dataset(
            file_list_txt=args.file_list, image_dir=args.image_dir, count=args.image_num
        )
        tuple_dataset =tuple(dataset)
        tuple_dataset_len =len(tuple_dataset)
        log.info("dataset len is:{}".format(tuple_dataset_len))
        image_num = max(args.image_num,tuple_dataset_len)
        ...
        for img, img_path in tqdm(tuple_dataset, total=image_num):
            infer_batch_size = (
