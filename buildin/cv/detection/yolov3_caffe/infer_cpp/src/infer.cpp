...
int main(int argc, char **argv) {
  ...
  // Set Output data ptr
  float *output_0_data_ptr =
      new float[output_tensors[0]->GetSize() /
                sizeof(output_tensors[0]->GetDataType())];
  int32_t *output_1_data_ptr =
      new int32_t[output_tensors[1]->GetSize() /
                  sizeof(output_tensors[1]->GetDataType())];

  // load images
  cout << "Load images..." << endl;
  std::vector<cv::String> image_paths = LoadImages(
      batch_size,
      FLAGS_image_dir);  //传入输入bs 防止模型可变bs 内部已处理好为bs的整数倍
  ...
  for (int i = 0; i <= image_num; i++) {
    if (i != 0 && i % batch_size == 0) {
      ...
      CNRT_CHECK(cnrtMemcpy(
          output_0_data_ptr, output_tensors[0]->GetMutableData(),
          output_tensors[0]->GetSize(), CNRT_MEM_TRANS_DIR_DEV2HOST));
      CNRT_CHECK(cnrtMemcpy(
          output_1_data_ptr, output_tensors[1]->GetMutableData(),
      ...
      for (int num_ = 0; num_ < batch_size; num_++) {
        ...
        std::vector<BBox> bboxes =
            Yolov3GetBBox(src_imgs[num_], scaling_factors[num_], img_h, img_w,
                          &output_0_data_ptr[num_ * output_0_size /
                                             (batch_size * sizeof(float))],
                          output_1_data_ptr[num_], FLAGS_confidence_thresholds);
        // post-process
        filelist << image_name[num_] << "\n";
        saveImgAndPreds(save_img, save_pred, src_imgs, image_name, save_img_dir,
                        save_pred_dir, labels, num_, bboxes);
      }
      if (i == image_num) {
        break;
      }
    ...
    if (src_img.data != NULL) {
      ...
      // pre-process
      scaling_factors[i % batch_size] =
          Preprocess(src_img, img_h, img_w, dst_img, FLAGS_pad_value);
      src_imgs[i % batch_size] = src_img;
      memcpy(input_data_ptr + (i % batch_size) * img_c * img_h * img_w,
             dst_img.data,
             img_c * img_h * img_w *
                 magicmind::DataTypeSize(
                     input_tensors[0]->GetDataType()));  //*sizeof(uint8_t)
                                                         //传输多少个字节的数据
    }
  }
  filelist.close();
  ...
  cout << "Throughput(1000 / execution time * image number): "
       << 1000 / execution_time.count() * image_num << "fps" << endl;
  cout << "Finished!" << endl;
  return 0;
}