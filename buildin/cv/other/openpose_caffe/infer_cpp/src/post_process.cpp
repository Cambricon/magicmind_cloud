...
// maximumpositivion: nms 0.02, paf 0.01 sample over 0.75 subset score 0.02,
// subset cnt 2 thresholds
...
static void RescaleKeypoints(Keypoints *keypoints, const float scaling_factor) {
...
class PostprocPose {
 public:
  ...
  std::pair<Keypoints, PersonInfos> Execute(cv::Mat img, float *preds,
                                            const float scaling_factor,
                                            const magicmind::Dims &input_dim,
                                            const magicmind::Dims &output_dim);
 ...
 private:
  Heatmaps GetHeatmaps(float *preds, const magicmind::Dims &input_dim,
                       const magicmind::Dims &output_dim);
  Keypoints GetKeypoints(const Heatmaps &heatmaps, const float scaling_factor);
  BodyPairs GetBodyPairs(const Heatmaps &heatmaps, const Keypoints &keypoints);
  PersonInfos GetPersonInfos(const Keypoints &keypoints,
                             const BodyPairs &body_pairs);

 private:
  cv::Mat NMSRegisterKernel(const cv::Mat &confidence_map);
  std::vector<KeypointInfo> AccuratePeakPositions(const cv::Mat &confidence_map,
                                                  const cv::Mat &nms_kernel,
                                                  const float scaling_factor);
  float GetPAFScore(const KeypointInfo &a, const cv::Mat &pafa,
                    const KeypointInfo &b, const cv::Mat &pafb);
...
std::pair<Keypoints, PersonInfos> PostprocPose::Execute(
...
PostprocPose::Heatmaps PostprocPose::GetHeatmaps(
    float *preds, const magicmind::Dims &input_dim,
    const magicmind::Dims &output_dim) {
...
Keypoints PostprocPose::GetKeypoints(const Heatmaps &heatmaps,
                                     const float scaling_factor) {
  // NMS
  Keypoints keypoints;
  keypoints.reserve(net_params_.nbody_parts);
  for (int body_part_idx = 0; body_part_idx < net_params_.nbody_parts;
       ++body_part_idx) {
...
PostprocPose::BodyPairs PostprocPose::GetBodyPairs(const Heatmaps &heatmaps,
  ...
  for (int pair_idx = 0; pair_idx < net_params_.nbody_part_pairs; ++pair_idx) {
    ...
    for (int ai = 0; ai < npeaksa; ++ai) {
      ...
      for (int bi = 0; bi < npeaksb; ++bi) {
        ...
        if (paf_score > 0.f) {
          body_pairs.emplace_back(BodyPairInfo(
              {cv::Point(part_idxa, ai), cv::Point(part_idxb, bi), pair_idx,
               paf_score, paf_score + 0.1f * (a.score + b.score)}));
        }
      }  // for peaksb
    }    // for peaksa
  }      // for body pairs
  std::sort(body_pairs.begin(), body_pairs.end(),
            [](const BodyPairInfo &a, const BodyPairInfo &b) {
              return a.total_score > b.total_score;
            });
  return body_pairs;
}

PersonInfos PostprocPose::GetPersonInfos(const Keypoints &keypoints,
                                         const BodyPairs &body_pairs) {
  PersonInfos persons;
  ...
  // A & B already assigned to different people, then the two people may be
  // combined into one, another one should be removed.
  ...
  for (const auto &body_pair : body_pairs) {
    ...
    if (assigneda < 0 && assignedb < 0) {
      ...
      // A & B already assigned to same person (circular/redundant PAF). update
      // score
      persons[assigneda].score += paf_score;
    } else if (assigneda >= 0 && assignedb >= 0 && assigneda != assignedb) {
      // A & B already assigned to different people. Merge to the first one if
      ...
      for (int ki = 0; ki < net_params_.nbody_parts; ++ki) {
        if (person1.keypoint_idxs[ki] != kInvalidPos &&
            person2.keypoint_idxs[ki] != kInvalidPos) {
          merge = false;
          break;
        }
      }
      if (merge) {
        ...
        for (int ki = 0; ki < net_params_.nbody_parts; ++ki) {
          if (person1.keypoint_idxs[ki] == kInvalidPos &&
            ...
            assigned[person2.keypoint_idxs[ki].x][person2.keypoint_idxs[ki].y] =
                assigned1;
          }
        }
  ...
  for (auto idx : persons_to_be_removed) persons.erase(persons.begin() + idx);

  // update score and remove if score less than kSubsetThreshold and number of
  // body pairs less than kMinSubsetCount
  for (auto iter = persons.begin(); iter != persons.end();) {
    float subset_score = iter->score / iter->nkeypoints;
    if (subset_score < kSubsetThreshold ||
        GetNumberBodyPairs(*iter) < kMinSubsetCount) {
...
cv::Mat PostprocPose::NMSRegisterKernel(const cv::Mat &confidence_map) {
  ...
  for (int x = 0; x < w; ++x) {
    for (int y = 0; y < h; ++y) {
      ...
      if (1 < x && x < (w - 2) && 1 < y && y < (h - 2)) {
        ...
        // border
        const float tl = (0 < x && 0 < y)
                             ? confidence_map.at<float>(y - 1, x - 1)
                             : kNMSThreshold;
        const float t =
            (0 < y) ? confidence_map.at<float>(y - 1, x) : kNMSThreshold;
        const float tr = (0 < y && x < (w - 1))
                             ? confidence_map.at<float>(y - 1, x + 1)
                             : kNMSThreshold;
        const float l =
            (0 < x) ? confidence_map.at<float>(y, x - 1) : kNMSThreshold;
        const float r =
            (x < (w - 1)) ? confidence_map.at<float>(y, x + 1) : kNMSThreshold;
        const float bl = (y < (h - 1) && 0 < x)
                             ? confidence_map.at<float>(y + 1, x - 1)
                             : kNMSThreshold;
        const float b =
            (y < (h - 1)) ? confidence_map.at<float>(y + 1, x) : kNMSThreshold;
        const float br = (x < (w - 1) && y < (h - 1))
                             ? confidence_map.at<float>(y + 1, x + 1)
                             : kNMSThreshold;
        if (score >= tl && score >= t && score >= tr && score >= l &&
            score >= r && score >= bl && score >= b && score >= br)
          kernel.at<uint8_t>(y, x) = 1;
      }  // else if border
    }    // for y
  }      // for x
...
std::vector<KeypointInfo> PostprocPose::AccuratePeakPositions(
    const cv::Mat &confidence_map, const cv::Mat &nms_kernel,
    const float scaling_factor) {
  const int w = confidence_map.cols;
  const int h = confidence_map.rows;
...
float PostprocPose::GetPAFScore(const KeypointInfo &a, const cv::Mat &pafa,
                                const KeypointInfo &b, const cv::Mat &pafb) {
  // L(p(u))
  ...
  const float max_distance =
      std::max(std::abs(distance.x), std::abs(distance.y));
  ...
  for (int i = 0; i < nsamples; ++i) {
    cv::Point sample;
    sample.x = std::max(
        0, std::min(w - 1, static_cast<int>(std::round(a.pos.x + x_step * i))));
    sample.y = std::max(
        0, std::min(h - 1, static_cast<int>(std::round(a.pos.y + y_step * i))));
    const float score =
        pafa.at<float>(sample) * norma + pafb.at<float>(sample) * normb;
...
int PostprocPose::GetNumberBodyPairs(const PersonInfo &person) {
  int count = 0;
