...
void COCOResultSaver::Write(cv::Mat img, const std::string &image_name,
  ...
  for (const auto &person : persons) Write(img, image_id, keypoints, person);
}

void COCOResultSaver::Write(cv::Mat img, int image_id,
                            const Keypoints &keypoints,
  ...
  for (int i : indexes_in_coco_order) {
    ...
    if (kInvalidPos == keypoint_idx) {
      ...
      auto point =
          RoundPoint(keypoints[keypoint_idx.x][keypoint_idx.y].pos, img);
      writer_.Int(point.x);
      writer_.Int(point.y);
      writer_.Int(1);
    }
  ...
  CHECK_EQ(true,
           writer_.Double(static_cast<int>(person.score * 100000) / 100000.0));
  writer_.EndObject();
}
