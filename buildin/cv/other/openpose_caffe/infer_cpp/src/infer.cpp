...
DEFINE_double(
    ...
    "The people with score greater thaan render_threshold will be rendered.");
  ...
  for (const auto &person : persons) {
    ...
    for (size_t i = 0; i < net_params.render_body_part_pairs.size(); ++i) {
      ...
      if (idxa != kInvalidPos && idxb != kInvalidPos) {
        ...
        if (a.score > FLAGS_render_threshold &&
            b.score > FLAGS_render_threshold)
          cv::line(img, RoundPoint(a.pos, img), RoundPoint(b.pos, img),
                   colors[i % ncolors], 2, 8, 0);
      }
    }  // for body pairs
    // draw keypoints
    for (const auto &idx : person.keypoint_idxs) {
      if (idx != kInvalidPos) {
        ...
        if (keypoint.score > FLAGS_render_threshold)
          cv::circle(img, RoundPoint(keypoint.pos, img), 2,
...
int main(int argc, char **argv) {
  ...
  if (image_paths.size() == 0) {
    LOG(INFO) << "No images found in dir [" << FLAGS_image_dir
              << "]. Support jpg.";
    return 0;
  }
  ...
  for (int i = 0; i < image_num; i += batch_size) {
    for (int j = 0; j < batch_size; j++) {
      ...
      LOG_EVERY_N(INFO, 100) << "Inference img: " << path << "\t\t\t" << i
                             << "/" << image_num << std::endl;
      ...
      // LOG(INFO) << "Inference img: " << image_name << "\t\t\t" << i << "/" <<
      // image_num << std::endl;
      float scaling_factor = Preprocess(img, input_dim, preprocess_data);
      scaling_factors.emplace_back(scaling_factor);
      // 8. copy in
      CNRT_CHECK(cnrtMemcpy((uint8_t *)(input_tensors[0]->GetMutableData()) +
                                j * (input_tensors[0]->GetSize() / batch_size),
                            preprocess_data,
                            input_tensors[0]->GetSize() / batch_size,
    ...
    if (!FLAGS_output_dir.empty()) {
      for (int j = 0; j < batch_size; j++) {
        auto res =
            Postprocess(images[j], (float *)output_cpu_ptrs + j * elem_count,
        ...
        if (FLAGS_save_img) {
          ...
          std::string save_path =
              FLAGS_output_dir + "/" + image_names[j] + "_rendered.jpg";
