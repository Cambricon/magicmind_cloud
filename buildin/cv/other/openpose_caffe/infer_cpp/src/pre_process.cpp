...
float Preprocess(cv::Mat img, const magicmind::Dims &input_dim, uint8_t *dst) {
  ...
  /*
     (x - mean) / std : This calculation process is performed at the first layer
     of the model, See parameter named [insert_bn_before_firstnode] in
     magicmind::IBuildConfig.
  */
  // resize as latter box
  int dst_h = input_dim[1];
  int dst_w = input_dim[2];
  float scaling_factor = std::min(1.0f * (dst_h - 1) / (img.rows - 1),
                                  1.0f * (dst_w - 1) / (img.cols - 1));
  ...
  cv::warpAffine(img, dst_mat, m, cv::Size(dst_w, dst_h),
                 (scaling_factor > 1.f ? cv::INTER_CUBIC : cv::INTER_AREA),
