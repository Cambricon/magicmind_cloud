...
if __name__ == "__main__":
    ...
    assert network.get_input(1).set_dimension(mm.Dims((100, 77))).ok()
    ...
    if args.shape_mutable=='true':
        ...
        assert config.parse_from_string('{"dim_range": {"0": {"min": [1, 3, 224, 224], "max": [64, 3, 224, 224]},"1": {"min": [100, 77], "max": [100, 77]}}}').ok()
    else:
    ...
    with mm.System() as mm_sys:
        ...
        if args.precision != "force_float32" and args.precision != "force_float16":
            print("Calibraing...")
            # 创建量化工具并设置量化统计算法
            calib_data.append(CalibData(mm.Dims((BATCH_SIZE, 3, INPUT_SIZE[0], INPUT_SIZE[1])), 0, MAX_SAMPLES))
            calib_data.append(CalibData(mm.Dims((BATCH_SIZE, 3, INPUT_SIZE[0], INPUT_SIZE[1])), 1, MAX_SAMPLES))
